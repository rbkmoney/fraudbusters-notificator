...
INSERT INTO ch_notificator.notification_template (name, type, skeleton, basic_params, query_text)
VALUES ('refund-by-captured', 'MAIL_FORM', '<>', 't,shopId,currency', 'SELECT t, shopId, currency, sm_ref AS refund, sm_all AS payment, sm_ref * 100 / sm_all AS metric ' ||
...
INSERT INTO ch_notificator.notification_template (name, type, skeleton, basic_params, query_text)
VALUES ('count decline/all > 70% by 1 week', 'MAIL_FORM', '<>', 'shopId', 'SELECT shopId,cnt,cnt_decline, cnt_decline * 100/cnt AS cnt_procent ' ||
                                                                          'FROM ( SELECT shopId, count(amount/100) AS cnt ' ||
                                                                          'FROM analytic.events_sink WHERE timestamp >= toDate($currentDate) - INTERVAL 1 week AND shopId != ''TEST'' AND status = ''captured'' AND currency = ''RUB'' ' ||
                                                                          'GROUP BY shopId ) ' ||
                                                                          'LEFT OUTER JOIN ' ||
...
INSERT INTO ch_notificator.notification_template (name, type, skeleton, basic_params, query_text)
VALUES ('Count fingerprint by all', 'MAIL_FORM', '<>', 't,fingerprint', 'SELECT timestamp AS t, fingerprint, count(fingerprint) AS cnt ' ||
                                                                         'FROM analytic.events_sink ' ||
...
VALUES ('>2 uniq shops by card', 'MAIL_FORM', '<>', 'cardToken', 'SELECT cardToken, count(invoiceId) AS invCnt ' ||
                                                                 ...
                                                                 'AND cardToken NOT IN (''WLCeYVOp4bW1hJ3oCv8KG'',''4xINpeuFJKxLPFGrdcrnUK'',''vUp3YTW8wcLTS0VttdebO'',''3zRjUhax48E1IY7wp7XZrS'',''2ttEW2bq4CpuBRD0VOWW9g'',''MfYHwQObqcIYlUTZD0v1y'',''4Y3xD7ftAkQrAwpEds5lMk'',''3q70aYBG6EH14ATRxxaom8'',''quvSmP2lQwjouDkbF84Op'',''4rqCK4KF5VwQL6o0TBzAHT'',''6I2L72hZrsc2SHUbDIVoZF'',''2X4iZv4CZ7XIG01DSgj4bv'',''70DVkKLfywEJGaj7oqUY1Q'',''7fAtmPASn44rNYieJfiv13'',''2CL3xaXNWq76hEGcVMj1rF'',''F3hq028Qsuo4KBNbRu8ud'',''4XZts3K0zWi7JAfU9kK8Ku'',''3csJuJyUbM7JmDCFiJoTfj'',''8yCcCMHbhUWbH9Kh6ZtLW'',''5iR9GSS7mzRFaYH1yROOiC'',''7MZOauIs8yUtoKuCamADWv'',''56kxngx6KgFFHrUbynNxi7'') ' ||
                                                                 'AND cardToken GLOBAL IN ( SELECT cardToken ' ||
                                                                 'FROM analytic.events_sink AS uniqCnt ' ||
...
INSERT INTO ch_notificator.notification_template (name, type, skeleton, basic_params, query_text)
VALUES ('demo >40k', 'MAIL_FORM', '<>', 't,cardToken,currency', 'SELECT timestamp AS t, cardToken, currency, max(amount / 100) AS maxAmount ' ||
                                                                'FROM analytic.events_sink ' ||
...
INSERT INTO ch_notificator.notification_template (name, type, skeleton, basic_params, query_text)
VALUES ('demo >70% failed 5 min', 'MAIL_FORM', '<>', 'shopId', 'SELECT shopId,cnt,cnt_decline, cnt_decline * 100/cnt AS cnt_procent ' ||
                                                               ...
                                                               'GROUP BY shopId ) ' ||
                                                               'LEFT OUTER JOIN ' ||
                                                               '( SELECT shopId, count(invoiceId) AS cnt_decline ' ||
                                                               'FROM analytic.events_sink ' ||
...
INSERT INTO ch_notificator.notification_template (name, type, skeleton, basic_params, query_text)
VALUES ('bin > 70 AND > 20 cnt', 'MAIL_FORM', '<>', 'bin,bankCountry', 'SELECT bin, bankCountry, cnt, cnt_decline, cnt_decline * 100/cnt AS cnt_procent ' ||
                                                                       'FROM ( SELECT bin, bankCountry, count(concat(invoiceId, paymentId)) AS cnt ' ||
                                                                       'FROM analytic.events_sink WHERE timestamp >= toDate($currentDate) - INTERVAL 2 day AND shopId != ''TEST'' AND status = ''captured'' AND currency = ''RUB'' ' ||
                                                                       'GROUP BY bin, bankCountry ) ' ||
                                                                       'LEFT OUTER JOIN ' ||
                                                                       '( SELECT bin, bankCountry, count(concat(invoiceId, paymentId)) AS cnt_decline ' ||
                                                                       'FROM analytic.events_sink where timestamp >= toDate($currentDate) - INTERVAL 2 day AND status = ''failed'' AND shopId != ''TEST'' AND ( errorCode=''authorization_failed:rejected_by_issuer'' OR errorCode=''authorization_failed:insufficient_funds'' OR errorCode=''preauthorization_failed:three_ds_not_finished'' OR errorCode=''no_route_found:risk_score_is_too_high'' ) ' ||
                                                                       'GROUP BY bin, bankCountry ) ' ||
                                                                       'USING bin, bankCountry ' ||
                                                                       'WHERE cnt_procent > 70 AND cnt + cnt_decline > 150');

INSERT INTO ch_notificator.notification_template (name, type, skeleton, basic_params, query_text)
VALUES ('Count fingerprint by shop', 'MAIL_FORM', '<>', 't,shopId,fingerprint', 'SELECT timestamp AS t, shopId, fingerprint, count(fingerprint) AS cnt ' ||
                                                                                'FROM analytic.events_sink ' ||
                                                                                'WHERE $currentDate <= timestamp ' ||
                                                                                'AND cardToken NOT IN (''WLCeYVOp4bW1hJ3oCv8KG'',''4xINpeuFJKxLPFGrdcrnUK'',''vUp3YTW8wcLTS0VttdebO'',''3zRjUhax48E1IY7wp7XZrS'',''2ttEW2bq4CpuBRD0VOWW9g'',''MfYHwQObqcIYlUTZD0v1y'',''4Y3xD7ftAkQrAwpEds5lMk'',''3q70aYBG6EH14ATRxxaom8'',''quvSmP2lQwjouDkbF84Op'',''4rqCK4KF5VwQL6o0TBzAHT'',''6I2L72hZrsc2SHUbDIVoZF'',''2X4iZv4CZ7XIG01DSgj4bv'',''70DVkKLfywEJGaj7oqUY1Q'',''7fAtmPASn44rNYieJfiv13'',''2CL3xaXNWq76hEGcVMj1rF'',''F3hq028Qsuo4KBNbRu8ud'',''4XZts3K0zWi7JAfU9kK8Ku'',''3csJuJyUbM7JmDCFiJoTfj'',''8yCcCMHbhUWbH9Kh6ZtLW'',''5iR9GSS7mzRFaYH1yROOiC'',''7MZOauIs8yUtoKuCamADWv'',''56kxngx6KgFFHrUbynNxi7'') ' ||
                                                                                'AND shopId != ''TEST'' AND fingerprint != '''' AND fingerprint != ''Other device'' AND notLike(fingerprint, ''Mozilla%'') AND notLike(fingerprint, ''Apache-HttpClient%'') ' ||
...
INSERT INTO ch_notificator.notification_template (name, type, skeleton, basic_params, query_text)
VALUES ('> 50k EUR-USD by cardToken 90 days', 'MAIL_FORM', '<>', 'cardToken, currency', 'SELECT cardToken, currency, sum(amount/100) AS sm ' ||
                                                                                        'FROM analytic.events_sink ' ||
                                                                                        'WHERE toDate($currentDate) - INTERVAL 90 day <= timestamp ' ||
                                                                                        'AND cardToken NOT IN (''WLCeYVOp4bW1hJ3oCv8KG'',''4xINpeuFJKxLPFGrdcrnUK'',''vUp3YTW8wcLTS0VttdebO'',''3zRjUhax48E1IY7wp7XZrS'',''2ttEW2bq4CpuBRD0VOWW9g'',''MfYHwQObqcIYlUTZD0v1y'',''4Y3xD7ftAkQrAwpEds5lMk'',''3q70aYBG6EH14ATRxxaom8'',''quvSmP2lQwjouDkbF84Op'',''4rqCK4KF5VwQL6o0TBzAHT'',''6I2L72hZrsc2SHUbDIVoZF'',''2X4iZv4CZ7XIG01DSgj4bv'',''70DVkKLfywEJGaj7oqUY1Q'',''7fAtmPASn44rNYieJfiv13'',''2CL3xaXNWq76hEGcVMj1rF'',''F3hq028Qsuo4KBNbRu8ud'',''4XZts3K0zWi7JAfU9kK8Ku'',''3csJuJyUbM7JmDCFiJoTfj'',''8yCcCMHbhUWbH9Kh6ZtLW'',''5iR9GSS7mzRFaYH1yROOiC'',''7MZOauIs8yUtoKuCamADWv'',''56kxngx6KgFFHrUbynNxi7'') ' ||
...
VALUES ('Old shops that start pay', 'MAIL_FORM', '<>', 'partyId, shopId', 'SELECT partyId, shopId, cntToday ' ||
                                                                          ...
                                                                          'FROM analytic.events_sink WHERE subtractDays(toDate($currentDate), 90) <= timestamp ' ||
                                                                          'AND cardToken NOT IN (''WLCeYVOp4bW1hJ3oCv8KG'',''4xINpeuFJKxLPFGrdcrnUK'',''vUp3YTW8wcLTS0VttdebO'',''3zRjUhax48E1IY7wp7XZrS'',''2ttEW2bq4CpuBRD0VOWW9g'',''MfYHwQObqcIYlUTZD0v1y'',''4Y3xD7ftAkQrAwpEds5lMk'',''3q70aYBG6EH14ATRxxaom8'',''quvSmP2lQwjouDkbF84Op'',''4rqCK4KF5VwQL6o0TBzAHT'',''6I2L72hZrsc2SHUbDIVoZF'',''2X4iZv4CZ7XIG01DSgj4bv'',''70DVkKLfywEJGaj7oqUY1Q'',''7fAtmPASn44rNYieJfiv13'',''2CL3xaXNWq76hEGcVMj1rF'',''F3hq028Qsuo4KBNbRu8ud'',''4XZts3K0zWi7JAfU9kK8Ku'',''3csJuJyUbM7JmDCFiJoTfj'',''8yCcCMHbhUWbH9Kh6ZtLW'',''5iR9GSS7mzRFaYH1yROOiC'',''7MZOauIs8yUtoKuCamADWv'',''56kxngx6KgFFHrUbynNxi7'') ' ||
                                                                          'AND shopId != ''TEST'' ' ||
                                                                          'GROUP BY partyId, shopId  )  ' ||
                                                                          'ANY LEFT JOIN  ' ||
...
INSERT INTO ch_notificator.notification_template (name, type, skeleton, basic_params, query_text)
VALUES ('limits', 'MAIL_FORM', '<>', 't,cardToken,currency', 'SELECT timestamp AS t, cardToken, currency, max(amount / 100) AS maxAmount, count() AS cnt, sum(amount / 100) AS sm_all ' ||
                                                             'FROM analytic.events_sink ' ||
                                                             'WHERE $currentDate <= timestamp AND status = ''captured'' AND shopId != ''TEST'' ' ||
                                                             'AND cardToken NOT IN (''WLCeYVOp4bW1hJ3oCv8KG'',''4xINpeuFJKxLPFGrdcrnUK'',''vUp3YTW8wcLTS0VttdebO'',''3zRjUhax48E1IY7wp7XZrS'',''2ttEW2bq4CpuBRD0VOWW9g'',''MfYHwQObqcIYlUTZD0v1y'',''4Y3xD7ftAkQrAwpEds5lMk'',''3q70aYBG6EH14ATRxxaom8'',''quvSmP2lQwjouDkbF84Op'',''4rqCK4KF5VwQL6o0TBzAHT'',''6I2L72hZrsc2SHUbDIVoZF'',''2X4iZv4CZ7XIG01DSgj4bv'',''70DVkKLfywEJGaj7oqUY1Q'',''7fAtmPASn44rNYieJfiv13'',''2CL3xaXNWq76hEGcVMj1rF'',''F3hq028Qsuo4KBNbRu8ud'',''4XZts3K0zWi7JAfU9kK8Ku'',''3csJuJyUbM7JmDCFiJoTfj'',''8yCcCMHbhUWbH9Kh6ZtLW'',''5iR9GSS7mzRFaYH1yROOiC'',''7MZOauIs8yUtoKuCamADWv'',''56kxngx6KgFFHrUbynNxi7'') ' ||
                                                             'GROUP BY t, cardToken, currency ' ||
...
INSERT INTO ch_notificator.notification_template (name, type, skeleton, basic_params, query_text)
VALUES ('> 600k EUR-USD by cardToken 90 days', 'MAIL_FORM', '<>', 'cardToken', 'SELECT cardToken, sum(amount/100) AS sm ' ||
                                                                               'FROM analytic.events_sink ' ||
                                                                               'WHERE toDate($currentDate) - INTERVAL 90 day <= timestamp ' ||
                                                                               'AND cardToken NOT IN (''WLCeYVOp4bW1hJ3oCv8KG'',''4xINpeuFJKxLPFGrdcrnUK'',''vUp3YTW8wcLTS0VttdebO'',''3zRjUhax48E1IY7wp7XZrS'',''2ttEW2bq4CpuBRD0VOWW9g'',''MfYHwQObqcIYlUTZD0v1y'',''4Y3xD7ftAkQrAwpEds5lMk'',''3q70aYBG6EH14ATRxxaom8'',''quvSmP2lQwjouDkbF84Op'',''4rqCK4KF5VwQL6o0TBzAHT'',''6I2L72hZrsc2SHUbDIVoZF'',''2X4iZv4CZ7XIG01DSgj4bv'',''70DVkKLfywEJGaj7oqUY1Q'',''7fAtmPASn44rNYieJfiv13'',''2CL3xaXNWq76hEGcVMj1rF'',''F3hq028Qsuo4KBNbRu8ud'',''4XZts3K0zWi7JAfU9kK8Ku'',''3csJuJyUbM7JmDCFiJoTfj'',''8yCcCMHbhUWbH9Kh6ZtLW'',''5iR9GSS7mzRFaYH1yROOiC'',''7MZOauIs8yUtoKuCamADWv'',''56kxngx6KgFFHrUbynNxi7'') ' ||
                                                                               'AND shopId != ''TEST'' AND status = ''captured'' AND (currency = ''EUR'' OR currency = ''USD'') ' ||
